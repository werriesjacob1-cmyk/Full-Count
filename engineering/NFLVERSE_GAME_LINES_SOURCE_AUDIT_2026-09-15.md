...
Classification: `NFLVERSE_PFR_CLOSING`
...
The source is useful as a retrospective closing-market control. The nflreadr schedule dictionary identifies `spread_line` and `total_line` as closing lines sourced from Pro-Football-Reference. The file does not identify an underlying sportsbook or record when the close was captured. It therefore cannot be represented as FanDuel history, used as a point-in-time prediction feature, or support book-specific calibration, precise closing-line value, or open-to-close research.
...
- Preserve `NFLVERSE_PFR_CLOSING` and `PRO_FOOTBALL_REFERENCE_VIA_NFLVERSE` on every derived row unless a separate primary source proves book identity.
- Hard-code `market_vintage=CLOSING`, `allowed_use=RETROSPECTIVE_BENCHMARK_CONTROL_ONLY`, and `point_in_time_feature_eligible=false`.
...
The next safe step is a normalized, source-labeled retrospective control and a leakage-audited model that never consumes the closing line as a prediction-time feature. Prospective FanDuel captures remain the authoritative substrate for FanDuel-specific evaluation.
