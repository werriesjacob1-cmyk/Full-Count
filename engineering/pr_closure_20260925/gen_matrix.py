...
D[215]=("**READY FOR APPROVAL** (after #208)","#208","test-only e2e reliability repair; independently reviewed (MINOR FIXES applied)","head root fails only on #208 fixture; integration main+#208+#215 `08155801a9` green","Claude","Jacob: merge right after #208")
D[216]=("**READY FOR APPROVAL** (after #208)","#208","docs-only verbatim preservation of #74/#77/#78/#84 preregs with provenance manifest","head root fails only on #208 fixture; in sequence tree `ace4cec2f4` green","Claude","Jacob: merge after #208; then #74/#77/#78 become superseded")
for n in (73,76): D[n]=("DEPENDENCY BLOCKED — Jacob decision (agent configuration)","—","SuperClaude activation: .claude agents/skills/rules/settings.json + CLAUDE.md edits","re-rooted; `.claude/settings.json` can change agent permissions, so porting needs explicit authorization; main has only `.claude/worktree-autosave.sh`","Jacob","decide port (as its own reviewed PR) or archive")
for n in (74,77,78): D[n]=("PORTED via #216 → SUPERSEDED once #216 merges","#216","pre-registration records (verbatim, provenance kept)","remaining branch content is obsolete generated output/data from the old tree","SUPERCHAD","after #216 merges, Jacob may close")
D[79]=("SCIENTIFIC REVIEW REQUIRED (portable)","—","MLB experiment-integrity primitives; no equivalent on main; its test passes on current main","unused on main; the MLB research program's status needs an owner decision","SUPERCHAD / Jacob","confirm program is active, then port as its own PR with review + CI")
D[80]=("SCIENTIFIC REVIEW REQUIRED (portable)","—","HR contact-state features; no equivalent on main; its test passes on current main (prereg dup ported via #216)","same","SUPERCHAD / Jacob","same")
D[81]=("SCIENTIFIC REVIEW REQUIRED (depends on #79)","#79","PA opportunity runner","its test needs `backtest.experiment_primitives` (#79)","SUPERCHAD / Jacob","port after #79")
D[82]=("SCIENTIFIC REVIEW REQUIRED (portable)","—","canonical artifact certifier + generation regime; no equivalent on main; both tests pass on current main","same as #79","SUPERCHAD / Jacob","same")
D[83]=("SCIENTIFIC REVIEW REQUIRED (portable)","—","HR no-intercept offset estimator; tests pass on current main","same","SUPERCHAD / Jacob","same")
D[84]=("SCIENTIFIC REVIEW REQUIRED (depends on #79)","#79, #80, #83","integrated HR contact-state Stage-1/2 stack (15 files); amended prereg ported via #216","stage tests need #79's primitives","SUPERCHAD / Jacob","port after #79/#80/#83")
D[85]=("REPAIR REQUIRED if revived (marked DO NOT MERGE)","—","PA-v1 prospective lifecycle (48 files); no equivalent on main","`test_prospective_capture.py` fails against current main (API drift: 'str' object has no attribute 'get'); 8 other tests pass","Claude / Jacob","archive unless PA-v1 is revived; then repair + review")
rows=[]
for p in C["prs"]:
