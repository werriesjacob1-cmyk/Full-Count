...
- Merge readiness: CI green on the reconciled head (see PR #135 for exact
  SHA), clean against current `main`. **Merge NOT performed** -- Jacob
  remains final authority per this mission's explicit instruction; PR
  prepared for his merge decision only.
