...
Source class: `NFLVERSE_PFR_CLOSING`

## Result

The digest-pinned `games.csv` source now has a deterministic research normalizer. nflreadr identifies its spread and total values as Pro-Football-Reference closing lines. A full replay normalized all 7,292 settled games and retained all 256 future or unsettled rows as explicit `UNSETTLED_GAME` exclusions. It did not infer any missing scores, lines, prices, sportsbook identity, or line timestamps.
...
Every normalized row carries the source repository, exact source commit, full-file SHA-256, acquisition timestamp, original game ID, `market_vintage=CLOSING`, `allowed_use=RETROSPECTIVE_BENCHMARK_CONTROL_ONLY`, and `point_in_time_feature_eligible=false`. The normalizer rejects or excludes:
