...
The input contract matches ``nfl.normalize.game_markets``. A complete snapshot
...
def seal_game_market_snapshot(
    ...
    *,
    ...
    for row in normalized_records:
        ...
        canonical = _text(row.get("canonical_market"), "record.canonical_market").lower()
    ...
    for row in normalized_failures:
        ...
        canonical = _text(row.get("canonical_market"), "failure.canonical_market").lower()
        if canonical not in MARKETS:
    ...
    body = {
        ...
        "records": sorted(normalized_records, key=lambda row: row["canonical_market"]),
        "failures": sorted(normalized_failures, key=lambda row: row["canonical_market"]),
        "research_only": True,
