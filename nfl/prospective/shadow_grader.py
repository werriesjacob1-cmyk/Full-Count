...
- the sealed snapshot hash is independently recomputed before any grading;
- capture/seal timing is revalidated as pre-kickoff evidence;
...
- duplicate exact outcome rows fail closed rather than choosing one.
"""
...
from nfl.prospective.shadow_snapshot import validate_pregame_timing
...
def verify_snapshot_integrity(snapshot: Mapping[str, Any]) -> str:
    """Recompute the v1 sealed snapshot hash and recheck pregame timing."""
    if not isinstance(snapshot, Mapping):
        raise ValueError("snapshot must be a mapping")
    expected = _text(snapshot.get("snapshot_sha256")).lower()
    ...
    if actual != expected:
        raise ValueError(
            f"snapshot hash mismatch: expected {expected}, recomputed {actual}"
        )

    records = snapshot.get("records")
    if not isinstance(records, list):
        raise ValueError("snapshot records must be a list")
    sealed_at = _text(snapshot.get("sealed_at"))
    if not sealed_at:
        raise ValueError("snapshot sealed_at is required")
    validate_pregame_timing(records, sealed_at)
    return actual
...
def _eligible(record: Mapping[str, Any]) -> bool:
    return (
        ...
        and _text(record.get("research_direction")).upper() in {"OVER", "UNDER"}
    )
...
def grade_snapshot(
    ...
    *,
    ...
    verified_sha = verify_snapshot_integrity(snapshot)
    records = snapshot.get("records")
    if not isinstance(records, list):
        raise ValueError("snapshot records must be a list")
    ...
    return {
        "grading_contract_version": 2,
        "evidence_class": "PROSPECTIVE_SHADOW_GRADED",
        "source_snapshot_sha256": verified_sha,
        "season": int(season),
        "season_type": str(season_type),
        "records": graded,
        "summary": {
            "records_total": len(graded),
            "outcomes_settled": sum(r["outcome_status"] == "SETTLED" for r in graded),
