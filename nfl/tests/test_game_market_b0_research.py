...
class GameMarketB0ResearchRunnerTests(unittest.TestCase):
    def test_pin_matches_audited_historical_source_contract(self):
        self.assertEqual(
            research.PINNED_SCHEDULE_SOURCE,
            {
    ...
    def test_history_includes_games_even_when_closing_line_missing(self):
        ...
        self.assertEqual(market[0]["source_class"], "NFLVERSE_PFR_CLOSING")
        self.assertEqual(
            market[0]["allowed_use"], "RETROSPECTIVE_BENCHMARK_CONTROL_ONLY"
        )
        self.assertFalse(market[0]["point_in_time_feature_eligible"])
