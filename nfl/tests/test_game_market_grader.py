...
def test_moneyline_tie_is_unresolved_not_guessed():
    result = grade_game_market(market(market_type="MONEYLINE", side="HOME", line=None), outcome(home_score=20, away_score=20))
    assert result["settlement"] == "UNRESOLVED_TIE"


@pytest.mark.parametrize("line,expected", [(-2.5, "HIT"), (-7.0, "PUSH"), (-7.5, "MISS")])
...
@pytest.mark.parametrize("side,line,expected", [("OVER", 46.5, "HIT"), ("OVER", 47.0, "PUSH"), ("UNDER", 47.5, "HIT"), ("UNDER", 46.5, "MISS")])
def test_total_settlements(side, line, expected):
    assert grade_game_market(market(market_type="TOTAL", side=side, line=line), outcome())["settlement"] == expected
