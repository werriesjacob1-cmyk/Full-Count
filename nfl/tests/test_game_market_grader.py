...
def moneyline():
    return market(canonical_market="moneyline", source_market_type="MONEY_LINE", market_id="ml-1", away_handicap=None, home_handicap=None)


def total(line=47.0):
    return market(canonical_market="game_total", source_market_type="TOTAL_POINTS_(OVER/UNDER)", market_id="total-1", total=line, away_handicap=None, home_handicap=None)
...
class GameMarketGraderTests(unittest.TestCase):
    ...
    def test_total_requires_numeric_line(self):
        with self.assertRaisesRegex(GameMarketGradeError, "total must be numeric"):
