...
class GameMarketB0Tests(unittest.TestCase):
    ...
    def test_non_control_market_provenance_fails_closed(self):
        rows = [
            scoring_row("g1", 2024, 5, "DEN", "KC", is_home=True, n=4, pf=24, pa=18),
            scoring_row("g1", 2024, 5, "KC", "DEN", is_home=False, n=4, pf=28, pa=22),
        ]
        bad = outcome("g1", 2024, 5, "DEN", "KC", 27, 20, 3.5, 44.5)
        bad["point_in_time_feature_eligible"] = True
        with self.assertRaisesRegex(GameMarketB0Error, "point_in_time_feature_eligible"):
            evaluate_against_closing_market(
                build_b0_predictions(rows), [bad], bootstrap_iterations=10
            )
