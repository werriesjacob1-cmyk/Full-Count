...
`neutral_dropback_rate_v1` is an explicit research definition, not a claim that
all public sites use the same filter:
- regular season
- 1st or 2nd down
...
- qb_dropback counts sacks and scrambles as dropbacks, matching nflfastR semantics

The current game's plays are aggregated first, but its summary enters rolling
...
def build_prior_pbp_tendencies(
    ...
    *,
    ...
    wp_low: float = 0.20,
    wp_high: float = 0.80,
    two_minute_seconds: int = 120,
) -> list[dict[str, Any]]:
    """Return team-game rows whose features contain only completed prior games."""
    ...
    for index, row in enumerate(rows):
        ...
        if missing:
            raise PbpPriorTendencyError(
                f"missing required PBP columns at row {index}: {', '.join(missing)}"
            )
        ...
        # nflfastR scrambles can be both rush_attempt=1 and qb_dropback=1.
        # Classify dropback first so the play is counted once and as pass intent.
        is_scrimmage = (dropback == 1 or rush == 1) and kneel == 0 and spike == 0
        is_neutral = (
            is_scrimmage
            and down in (1, 2)
            and wp_low <= wp <= wp_high
            and half_seconds > two_minute_seconds
        )
    ...
    for summary in games.values():
        if len(summary["opponents"]) != 1:
            raise PbpPriorTendencyError(
                f"game/team has {len(summary['opponents'])} opponents: {summary['game_id']} {summary['team']}"
            )
        if summary["scrimmage_plays"] <= 0:
            raise PbpPriorTendencyError(
                f"game/team has no qualifying scrimmage plays: {summary['game_id']} {summary['team']}"
            )
        game_rows.append({
            **{k: v for k, v in summary.items() if k != "opponents"},
            "opponent_team": next(iter(summary["opponents"])),
        })
    ...
    for current in game_rows:
        ...
        output.append({
            ...
            "prior_scrimmage_plays_n": prior_scrimmage,
            "prior_dropbacks_n": prior_dropbacks,
            "prior_neutral_plays_n": prior_neutral,
            ...
            "prior_neutral_dropback_rate_v1": (
                prior_neutral_dropbacks / prior_neutral if prior_neutral else None
            ),
            "prior_mean_scrimmage_plays_per_game": (
                prior_scrimmage / len(history) if history else None
            ),
            "neutral_definition": {
                "downs": [1, 2],
                "wp_low_inclusive": wp_low,
                "wp_high_inclusive": wp_high,
                "minimum_half_seconds_exclusive": two_minute_seconds,
                "kneels_excluded": True,
                "spikes_excluded": True,
                "scrambles_classified_as_dropbacks": True,
            },
