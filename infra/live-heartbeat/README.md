...
## Required Jacob setup (stop-here boundary)

This Worker cannot deploy or run without three things only you can do:

### 1. Cloudflare account + Worker deployment
1. Sign in (or create a free account) at https://dash.cloudflare.com.
2. From `infra/live-heartbeat/`, run `npx wrangler login` (opens a
   browser to authorize the CLI) -- one-time, local to whichever machine
   deploys this.
3. Run `npx wrangler deploy`. This publishes the Worker and registers its
   `*/5 * * * *` Cron Trigger. No further dashboard clicking is required
...
5. Set an expiration (recommend 90 days; GitHub will remind you to
   rotate it -- rotating means repeating step 3 below with the new value,
   nothing else changes).
6. Generate, copy the token once (GitHub will not show it again).

### 3. Cloudflare secret
From `infra/live-heartbeat/`:
```
npx wrangler secret put GITHUB_PAT
```
Paste the token when prompted. This stores it encrypted in Cloudflare,
not in any file in this repo.

**Nothing beyond these three steps is required from you to make the
heartbeat live.** Everything else (the Worker code, its tests, the
wrangler config) is already in this directory.
